...
<script type="text/javascript">
	function jump() {
		...
		location.href = "${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum="
				+ pNum;
      ...
      <div class="row-fluid">
        <div class="span12">
          <div class="widget-box">
			...
			<c:if test="${empty pageBean.usersCustoms}">
		    <h1>没有用户</h1>
	         </c:if>
	          <c:if test="${not empty pageBean.usersCustoms}">
            <form action ="${pageContext.request.contextPath }/admins/findUsersCustomByCondition.action" method="post" role="form">
            ...
            <div class="widget-content nopadding">
              <table class="table table-bordered data-table">
                ...
                <tbody>
                  ...
                  <c:forEach items="${pageBean.usersCustoms}" var="usersCustom">
				<tr class="gradeX">
					<td>${usersCustom.username }</td>
					<td>${usersCustom.email }</td>
					<td>${usersCustom.phone }</td>
					<td>${usersCustom.male }</td>
				    <td>
				    <date:date value ="${usersCustom.created}"/>
				    </td>
				    <td><date:date value ="${usersCustom.loged}"/></td>
				    <td>${usersCustom.ip}</td>
				    <td>${usersCustom.nickname }</td>
				    <td>${usersCustom.name }</td>
					<td><a href="${pageContext.request.contextPath }/admins/findUsersCustomauthorityByUid.action?uid=${usersCustom.uid}">更改权限</a>&nbsp;
					&nbsp;<a href="${pageContext.request.contextPath }/admins/delUsersCustomByUid.action?uid=${usersCustom.uid}">删除</a>
					<a href="${pageContext.request.contextPath }/admins/findUsersCustomMailboxByUid.action?uid=${usersCustom.uid}">发通知给用户</a></td>						                           
				</tr>
			</c:forEach>
                </tbody>
              </table>
              ...
              <div class="pagination alternate text-center">
               <tr>
				<td colspan="9" align="center">
					<!-- 显示首页 --> <c:if test="${pageBean.pNum==1}">
						...
						<a
							href="${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum=1">首页</a>
						<a
							href="${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum=${pageBean.pNum-1 }">上一页</a>
               ...
               ${i}
						...
						<c:if test="${pageBean.pNum!=i}">
							<a
								href="${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum=${i} ">${i}
							</a>
						</c:if>
						...
						<a
							href="${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum=${pageBean.pNum+1}">下一页</a>
						<a
							href="${pageContext.request.contextPath }/admins/findAllUsersCustom.action?pNum=${ pageBean.totalPageNum}">尾页</a>
