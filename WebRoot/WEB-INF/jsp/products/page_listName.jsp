		...
		<div id="content-header">
			<div id="breadcrumb">
				<a href="index.html" title="返回主页" class="tip-bottom"><i
					...
					href="goods-approved.html">通过审核的商品</a>
			</div>
			<h1>通过审核的商品</h1>
		</div>
		<!--End-breadcrumbs-->
						...
						<form
							...
							<div class="form-group">
								...
								<select name="conditionName" id="input" class="form-control"
									...
									<option value="pcname">商品分类</option>
							...
							<div class="widget-content nopadding">
								...
								<div class="pagination alternate text-center">
									<ul>
										...
										<c:if test="${pageBean.pNum != 1}">
											<li><a href="${pageContext.request.contextPath }/findByPageByName.action?status=${pageBean.status}&conditionValue=${pageBean.conditionValue}&conditionName=${pageBean.conditionName}&pNum=1">首页</a></li>
											<li><a
												href="${pageContext.request.contextPath }/findByPageByName.action?status=${pageBean.status}&conditionValue=${pageBean.conditionValue}&conditionName=${pageBean.conditionName}&pNum=${pageBean.pNum-1 }">
													上一页</a></li>
										</c:if>
										<!-- 当前页为中心前后各显示10页 -->
										...
										<c:forEach begin="${begin}" end="${end}" var="i">
											...
											<c:if test="${pageBean.pNum!=i}">
												<li><a
													href="${pageContext.request.contextPath }/findByPageByName.action?status=${pageBean.status}&conditionValue=${pageBean.conditionValue}&conditionName=${pageBean.conditionName}&pNum=${i }">${i
														} </a></li>
											</c:if>
										</c:forEach>
										<!-- 显示尾页 -->
										...
										<c:if test="${pageBean.pNum!=pageBean.totalPageNum}">
											<li><a
												href="${pageContext.request.contextPath }/findByPageByName.action?status=${pageBean.status}&conditionValue=${pageBean.conditionValue}&conditionName=${pageBean.conditionName}&pNum=${pageBean.pNum + 1 }">
													下一页</a></li>
											<li><a
												href="${pageContext.request.contextPath }/findByPageByName.action?status=${pageBean.status}&conditionValue=${pageBean.conditionValue}&conditionName=${pageBean.conditionName}&pNum=${pageBean.totalPageNum}">
													尾页</a></li>
										</c:if>
										<input type="text" id="pNum" size="2" />
