...
<body> <!--Header-part-->
  ...
  <div id="content">
    ...
    <div id="content-header">
      <div id="breadcrumb">
        <a href="index.html" title="返回主页" class="tip-bottom"><i class="icon-home"></i> Home</a><a href="goods-detail.html">商品详情</a>
      </div>
      <h1>商品详情</h1>
    </div>
    <!--End-breadcrumbs-->
    <div class="container-fluid">
	 ...
	 <div class="span6">
			...
			<tr>
				<td>商品类型</td>
				<td><strong>${productsExtend.type }</strong></td>
			</tr>
			<tr>
				<td>售价</td>
			...
			<tr>
				<td>浏览次数</td>
				<td><strong>${productsExtend.views }</strong></td>
			</tr>
			<tr>
				<td>评论次数</td>
				<td><strong>${productsExtend.commentcount }</strong></td>
			</tr>
				<tr>
		...
		<tr>
			<td>
			<a href="${pageContext.request.contextPath }/insertOrderitemsSuccess.action?pid=${productsExtend.pid}">抢购</a>
			</td>
			</tr>
...
<form action="">
		<table width="750" height="60" cellpadding="0" cellspacing="0"
			border="0">
			<c:forEach items="${productsExtend.imagesExtend}" var="imagesExtend">
				<div>
					<dl>

						<dt>
							<img src="${pageContext.request.contextPath }${imagesExtend.url}"
...
<!-- 列出所有的评论 -->
<!-- 显示list数据 -->
	<c:if test="${empty pageBean.product_commentsExtend}">
		<h1>无商品评论！</h1>
	</c:if>
	<c:if test="${not empty pageBean.product_commentsExtend}">
		<table border="1" width="100%">
			...
			<c:forEach items="${pageBean.product_commentsExtend }" var="item">
				<tr>
					<td>${item.uid }</td>
					 <td><date:date value="${item.created }"/></td>
					<td>${item.comment }</td>
				</tr>
			...
			<tr>
				...
				<td colspan="9" align="center">
					...
					首页  上一页
						...
						<a
							href="${pageContext.request.contextPath }/findCommentByPage.action?pid=${productsExtend.pid }&pNum=1">首页</a>
						<a
							href="${pageContext.request.contextPath }/findCommentByPage.action?pid=${productsExtend.pid }&pNum=${pageBean.pNum-1 }">上一页</a>
					...
					<!-- 判断前面有没有10页 --> <c:if test="${pageBean.pNum-10>0}">
						...
						<c:if test="${pageBean.pNum!=i}">
							<a
								href="${pageContext.request.contextPath }/findCommentByPage.action?pid=${productsExtend.pid }&pNum=${i }">${i
								} </a>
						</c:if>
					...
					下一页 尾页
						...
						<a
							href="${pageContext.request.contextPath }/findCommentByPage.action?pid=${productsExtend.pid }&pNum=${pageBean.pNum + 1 }">下一页</a>
						<a
							href="${pageContext.request.contextPath }/findCommentByPage.action?pid=${productsExtend.pid }&pNum=${pageBean.totalPageNum}">尾页</a>
